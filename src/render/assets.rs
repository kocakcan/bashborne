...
impl Assets {
    pub async fn load() -> Self {
        ...
        let walk_frames =
            Texture2D::from_file_with_format(include_bytes!("../../assets/walk_frames.png"), None);
        walk_frames.set_filter(FilterMode::Nearest);

        // Linear-filtered, unlike the tile/character sheets: those are
        // low-res pixel art that wants a hard Nearest edge, but Agave Nerd
        // Font (which replaced the earlier Kenney Mini Square pick) is a
        // smooth vector font — `flush_text` already rasterizes it at its
        // true final on-screen size rather than blowing up a low-res atlas,
        // so it reads as a normal crisp-but-antialiased UI font instead of
        // pixel art. Paired with the plain-alpha fragment shader below
        // (no hard threshold), Nearest here would stair-step every curved
        // stroke (O/B/R/etc.) since it throws away the atlas's antialiasing.
        let mut font = load_ttf_font_from_bytes(include_bytes!("../../assets/agave_nerd_font.ttf"))
...
/// Passes the font atlas's own anti-aliased alpha straight through. This
/// used to hard-threshold it (`step(0.5, a)`) to make the old blocky Kenney
/// Mini Square font read like crisp pixel art, but Agave Nerd Font's round
/// strokes (O/B/R/etc.) turn into a visible staircase without real AA — a
/// smooth vector font should just look like a normal antialiased font.
...
/// The three frames of the synthesized walk cycle (see
/// `assets/walk_frames.png`, generated by a one-off Python/Pillow script
/// since no walk-cycle art exists in the bundled Kenney sheet) — a quick
/// left/right leg shuffle around a resting pose, cycled by
/// `ExploreState::step_elapsed` while the player is mid-step.
...
/// Atlas rect (in `walk_frames`) for the player's overworld sprite at a
/// given point in its walk cycle. Row 0 (and only row) of `walk_frames.png`
/// — NPCs stay on their static `npc_rect` frame, only the player animates.
pub fn player_walk_rect(frame: WalkFrame) -> Rect {
    cell(frame as u32, 0)
}
