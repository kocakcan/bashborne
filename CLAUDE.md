...
`game/*.rs`) and exercise game logic directly (turn order, damage,
weapon/rarity math, loot and shop pricing, boss scripted moves,
inventory/shop input handling, save/load round-tripping) without needing a
window. `render/*.rs` has no automated tests.

**If you need to verify rendering itself**: see the `playtest` skill
(`.claude/skills/playtest/SKILL.md`), which drives the game and captures
screenshots in-process. Falling back to manual `cargo run` + eyeballing (or
an OS screenshot) is always an option too.
